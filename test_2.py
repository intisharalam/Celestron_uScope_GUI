...
# Cap every window to this width so windows are directly comparable --
# without this, a higher-resolution frame just shows up as a BIGGER window,
# which tricks the eye into thinking there's more detail when really
# there's just more screen space being used to show the same picture.
DISPLAY_WIDTH = 640
...
print("\nShowing comparison windows. Press any key to close and exit.")
print(f"Every window is capped to {DISPLAY_WIDTH}px wide, so they're a fair")
print("size-for-size comparison -- bigger windows don't just mean more detail.")

for label, (frame, actual_w, actual_h) in captured.items():
    if frame is None:
        continue
    view = resize_to_width(frame, DISPLAY_WIDTH)
    cv2.imshow(f"{label}  [captured at {actual_w}x{actual_h}]", view)
